...
posseqarray=readfas("pos_W2_peak_201bp_summit_peaks_maskedNfilt.fa")
...
#############negative
negseqarray=readfas("neg_W2_peak_201bp_summit_peaks_maskedNfilt.fa")
...
model.add(layers.Conv1D(filters=512, kernel_size=20, activation='relu', padding='same',input_shape=(201,4)))
model.add(layers.Dropout(rate=0.1))

#model.add(layers.Dense(units=1,activation='relu'))
model.add(layers.Conv1D(filters=64,kernel_size=8, padding='same',activation='relu'))
...
checkpointer = ModelCheckpoint(filepath='/scrfs/storage/ratnayak/data/DeepLearningLifeSciences/cross_soy_mod/soy_mod/SOY_CNNLSTM_SIGM_W2_mskedfilt_v9.h5',verbose=1, save_best_only=True)
...
model.save('/scrfs/storage/ratnayak/data/DeepLearningLifeSciences/cross_soy_mod/soy_mod/SOY_CNNLSTM_SIGM_W2_mskedfilt_v9.h5', overwrite=True)
...
fig.savefig("soy_WRKY_test_confusion_CNNLSTM_SIGM_mskedfilt_v9.png")
...
# Setting Parameters
    ...
    plt.legend(loc='best')
    #plt.show()
    fig1.savefig("WRKY_acc_curve_mskedfilt_v9.png")
    ...
    plt.legend(loc='best')
    fig2.savefig("WRKY_loss_mskedfilt_v9.png")
    #plt.show()
...
fpr, tpr, thresh = metrics.roc_curve(rytest, result_map, pos_label=1)

#
## roc curve for tpr = fpr
random_probs = [0 for i in range(len(rytest))]
p_fpr, p_tpr, _ = metrics.roc_curve(rytest, random_probs, pos_label=1)

auc = metrics.roc_auc_score(rytest, result_map)
...
fig3.savefig("WRKY_ROCAUC_v9.png",dpi=400)
#plt.plot(fpr,tpr, auc="+str(auc))
#plt.legend(loc=4)
#plt.show()
...
#################<<<<<<<<<<<<<<<<<<auPCR>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
precision, recall, _ = precision_recall_curve(rytest, result_map)
fig4 = plt.figure()
...
#
#
plt.title("auPRC: " + str(average_precision_score(rytest, result_map)))
fig4.savefig("WRKY_auPRC_mskedfilt_v9.png")
